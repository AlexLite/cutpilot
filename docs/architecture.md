...
The first implementation uses Python's standard library HTTP server. `/api/plan` sends only the selected basename, byte size, and user task to the configured OpenRouter-compatible adapter. The server validates the returned command token array, constructs the CutPilot filename itself, and stores the validated plan in memory. `/api/jobs` requires an explicit confirmation and verifies the source size, modification time, filesystem change time, and local content fingerprint have not changed; it copies the source to a random `.part` file in the watcher directory, fsyncs it, and atomically renames it to the final queue filename. It never deletes the source.
