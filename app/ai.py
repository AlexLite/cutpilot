...
class OpenRouterAdapter:
    ...
    def create_plan(self, source_filename: str, metadata: dict[str, Any], task: str, feedback: str | None = None) -> dict[str, Any]:
        ...
        system = (
            ...
            "-logo|-nl|-nologo|-nc|-nocut|-mute, -crp-START-END, -crp=START-END, "
            ...
            "Translate only operations explicitly requested by the user; never invent a format, resolution, FPS, logo, audio, size, or edit. "
            "Return at most one container, one resolution, one FPS, one target size, one audio/logo operation, and one edit command. "
            "If a requested operation is unclear, omit it instead of guessing. The summary must be short and entirely in Russian. "
