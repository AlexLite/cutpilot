...
_NOISE = re.compile(r"\b(?:сделай|сделать|обработай|обработать|перекодируй|перекодировать|конвертируй|конвертировать|пересчитай|пересчитать|сожми|сжать|уменьши|уменьшить|сократи|сократить|собери|собрать|обрез\w*|видео|файл|формат|разрешение|размер|до|примерно|и|в|на|по|для|мне|пожалуйста|без|наложи|наложить|склей|склеить|соедини|соединить|объедини|объединить|оставь|оставить|таймкод\w*|мегабайт\w*|гигабайт\w*|логотип\w*|лого|с|c|cut)\b", re.I)
