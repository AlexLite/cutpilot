...
_NOISE = re.compile(r"\b(?:сделай|сделать|обработай|обработать|перекодируй|перекодировать|конвертируй|конвертировать|пересчитай|пересчитать|сожми|сжать|ужми|ужать|уменьши|уменьшить|сократи|сократить|собери|собрать|обрез\w*|видео|файл|формат|разрешение|размер|до|примерно|и|в|на|по|для|мне|пожалуйста|без|наложи|наложить|приклей|приклеить|склей|склеить|соедини|соединить|объедини|объединить|оставь|оставить|таймкод\w*|мегабайт\w*|гигабайт\w*|логотип\w*|лого|с|c|cut)\b", re.I)
...
_MUTE_PHRASE = r"(?:убить\s+звук|убери?\s+звук|убрать\s+звук|замьютить|замутить|мьют|mute|сделать\s+тишину)"
...
def simple_plan(task: str, duration_seconds: float | str | None = None) -> dict[str, Any] | None:
    ...
    if re.search(_MUTE_PHRASE, text):
        ...
        consumed = re.sub(_MUTE_PHRASE, " ", consumed)

    ranges = list(re.finditer(_RANGE, text))
    if ranges:
        if re.search(r"(?:склей|соедини|объедини|собери|собрать|concat)", text):
